...
} from "../../src/features/platform-builder-v2/forms/forms-actors";
import { getFormsPlaceholderObject } from "../../src/features/platform-builder-v2/forms/forms-placeholder-data";
...
describe("platform builder forms actor access", () => {
  ...
  it("blocks view-only editors from structure actions while keeping delegated view actions available", () => {
    ...
    expect(access.structureRestrictionKey).toBe("tenant.platformBuilder.forms.permission.lockedModelOwnerOnly");
    expect(access.summaryKey).toBe("tenant.platformBuilder.forms.canEditScreensOnly");
  });
  ...
  it("blocks view-only editors when a model does not grant view-only editing", () => {
    ...
    expect(access.summaryKey).toBe("tenant.platformBuilder.forms.permissionSummary.viewAccessUnavailable");
    expect(access.viewRestrictionKey).toBe("tenant.platformBuilder.forms.permission.viewAccessDisabled");
  });
  ...
  it("keeps readonly users able to open workspaces while blocking mutations", () => {
    ...
    expect(access.canDeleteModel).toBe(false);
    expect(access.canDeleteView).toBe(false);
    expect(access.structureRestrictionKey).toBe("tenant.platformBuilder.forms.permission.readonly");
    expect(access.viewRestrictionKey).toBe("tenant.platformBuilder.forms.permission.readonly");
  });
});
