...
- Actual: Dedicated per-lookup settings/filter review is partially implemented. Generic lookup active filtering, preset lookup authoring controls, the shared dictionary option query contract, runtime named-dictionary filter enforcement, first-pass contact lookup View Filter authoring, and backend runtime-list compilation for single-value `contact_lookup` user/company clauses now exist. Non-contact lookup View Filter compilers and multivalue lookup filtering remain future work.
