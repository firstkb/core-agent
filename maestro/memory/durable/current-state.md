...
- `landed` `code-confirmed`: Navigation Builder runtime visibility filtering is active for `/app/navigation`. The backend evaluator reads derived runtime/access rows, resolves the current tenant user against `users`/`company`/`companytype`/`jobtype`, applies `users OR ((companies OR company types) AND job types)`, enforces parent-bounds-child narrowing, filters app menu and utility rail entries, supports protected `root_only` access, lets root/admin claims bypass access policies while still respecting inactive filtering, rejects non-root saves when saved or incoming definitions contain root-only access, and suppresses empty/consecutive/trailing menu titles after filtering.
...
- `planned` `owner-confirmed`: Navigation Builder direct route/API ACL enforcement remains planned. Runtime sidebar/utility rail visibility is now filtered, but direct Form View/App Page APIs must still be guarded by the same derived evaluator before access is complete.
