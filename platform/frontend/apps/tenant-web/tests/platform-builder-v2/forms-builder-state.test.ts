...
import { getFormsAuthoringAccess, getFormsPlaceholderActor } from "../../src/features/platform-builder-v2/forms/forms-actors";
import {
...
} from "../../src/features/platform-builder-v2/forms/forms-builder-state";
import {
  getFormsPlaceholderObject,
  getFormsPlaceholderScreen,
} from "../../src/features/platform-builder-v2/forms/forms-placeholder-data";
...
describe("forms builder workspace state", () => {
  ...
  it("falls back to legacy saved documents when no new snapshot exists", () => {
    ...
    window.localStorage.setItem(
      `tenant-web-platform-builder-v2-screen-document:${editableObject.id}:${editableScreen.id}`,
      JSON.stringify({
        ...document,
        currentParentId: nestedSectionId,
        selectedNodeId: document.nodes.find((node) => node.type === "field")?.id ?? document.selectedNodeId,
