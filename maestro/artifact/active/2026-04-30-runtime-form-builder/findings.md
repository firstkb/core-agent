...
- Actual: Dedicated per-lookup settings/filter review is partially implemented. Generic lookup active filtering, preset lookup authoring controls, the shared dictionary option query contract, and runtime named-dictionary filter enforcement now exist. Lookup-aware View filter compiler/UX improvements remain future work.
