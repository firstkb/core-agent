...
; (function (global) {
    ...
    function resolveTTSMode(manifest) {
        var mode = ((manifest && manifest.tts) || 'auto');
        if (typeof mode === 'string') mode = mode.toLowerCase();

        if (mode === 'mp3') return false;
        if (mode === 'webspeech') return hasWebSpeech();
        // auto
        return hasWebSpeech();
    }

    function hasWebSpeechSTT() {
        try { return !!(global.SpeechRecognition || global.webkitSpeechRecognition); }
        catch { return false; }
    }

    // STT mode resolver: manifest.stt = 'auto' | 'openai' (default 'auto')
    function resolveSTTMode(manifest) {
        var m = (manifest && manifest.stt) ? String(manifest.stt).toLowerCase() : 'auto';
        if (m === 'openai') return 'openai';
        return hasWebSpeechSTT() ? 'browser' : 'openai';
    }

    // Default languages
    var DEFAULT_STT_LANG_SERVER = 'en';    // what we send to backend
    var DEFAULT_STT_LANG_BROWSER = 'en-US';// BCP-47 for SpeechRecognition
    function normalizeBCP47(lang) {
        var s = String(lang || '').trim();
        return (s.length >= 4) ? s : DEFAULT_STT_LANG_BROWSER;
    }
    ...
    AskMp3.prototype.sayText = async function (text, opts) {
        if (!this.opts.useTTSFallback || !('speechSynthesis' in window)) return;

        try { speechSynthesis.cancel(); } catch { }
        var u = new SpeechSynthesisUtterance(String(text || ''));
        try {
            u.lang = (opts && opts.lang) || document.documentElement.lang || navigator.language || 'en-US';
        } catch { }
        return new Promise(function (r) {
            u.onend = r; u.onerror = r; speechSynthesis.speak(u);
        });
    };
    AskMp3.prototype.canTTS = function () {
        return !!(this.opts.useTTSFallback && ('speechSynthesis' in window));
    };
    AskMp3.prototype.ask = function (node) {
        if (!node) return Promise.resolve();
        var hasText = !!(node.text && String(node.text).trim().length);
        var hasMp3 = !!node.mp3;
        if (this.canTTS() && hasText) return this.sayText(node.text);
        if (hasMp3) return this.sayUrl(node.mp3);
        if (hasText) return this.sayText(node.text);
    ...
    Recorder.prototype.startVisualOnly = async function () {
        if (!this.visualVAD || !this.canvas || !this.ctx) return { stop: function () { } };
        if (this._viz && this._viz.running) return this._viz;

        try { await this.ensureMic(); } catch (e) {
        ...
        var energyEMA = 0, noiseFloor = 2, START_FACTOR = 2.2;
        var rafId = 0, last = performance.now();
        var g = this.ctx, c = this.canvas;
        ...
        var loop = () => {
            ...
            energyEMA = 0.6 * energyEMA + 0.4 * rms;
            noiseFloor = 0.995 * noiseFloor + 0.005 * energyEMA;
            var thrStart = Math.max(5, noiseFloor * START_FACTOR);
        ...
        this._viz = { stop: stop, running: true };
        return this._viz;
    };

    Recorder.prototype.stopVisualOnly = function () {
        if (this._viz) {
            try { this._viz.stop && this._viz.stop(); } catch { }
        }
        this._viz = null;
    };
    ...
    function Transport(opts) {
        this.endpoint = (opts && opts.endpoint) ? opts.endpoint : 'openai_transcribe';
    ...
    // Browser STT (Web Speech API) — one-shot
    Transport.prototype.recognizeBrowserOnce = function (opts) {
        var Rec = global.SpeechRecognition || global.webkitSpeechRecognition;
        if (!Rec) return Promise.resolve({ text: '', confidence: 0, error: 'no-rec' });
        var lang = normalizeBCP47((opts && opts.lang) || DEFAULT_STT_LANG_BROWSER);
        return new Promise((resolve) => {
            ...
            r.maxAlternatives = 1;
            var finished = false, lastError = '';

            var cleanup = () => { this._speechRec = null; };
            var to = setTimeout(function () { try { r.abort(); } catch { } }, (opts && opts.timeoutMs) || 10000);
            ...
            r.onresult = function (ev) {
                try {
                    var res = ev.results && ev.results[0] && ev.results[0][0];
                    var text = (res && res.transcript) || '';
                    var conf = (res && (res.confidence != null)) ? res.confidence : 1;
                    done({ text, confidence: conf, error: '' });
                } catch { done({ text: '', confidence: 0, error: 'result-ex' }); }
            };
            r.onerror = function (e) { lastError = (e && e.error) || 'error'; done({ text: '', confidence: 0, error: lastError }); };
            r.onend = function () { done({ text: '', confidence: 0, error: lastError || 'ended' }); };
            try { r.start(); } catch { done({ text: '', confidence: 0, error: 'start-ex' }); }
        });
    };

    Transport.prototype.abortBrowser = function () {
        try { this._speechRec && this._speechRec.abort(); } catch { }
        this._speechRec = null;
    };
    ...
    function VoiceCore(options) {
        ...
        this._askingQuestion = false;
        this._reaskOnResume = false;
        this._inQuestionCycle = false;
        ...
        this._killed = false;          // destroy()/unmount()
        this._stoppedByUser = false;   // Stop/voice STOP
    ...
    VoiceCore.prototype._mp3On = function () {
        try { return !this.ask || !this.ask.canTTS(); } catch { return true; }
    };
    ...
    VoiceCore.prototype.init = async function () {
        ...
        try { this.ask.opts.useTTSFallback = resolveTTSMode(this.manifest); } catch { }
        this.categories = normalizeManifest(this.manifest);

        this.fields = normalizeFields(this.manifest);

        // STT setup (RESPECT manifest.stt)
        var pref = (this.manifest && typeof this.manifest.stt === 'string')
            ? String(this.manifest.stt).toLowerCase()
            : 'auto';

        this.sttPref = (pref === 'openai' || pref === 'browser') ? pref : 'auto';

        this.allowOpenAIFallback = (this.sttPref === 'auto');

        if (this.sttPref === 'openai') {
            this.sttMode = 'openai';
        } else if (this.sttPref === 'browser') {
            this.sttMode = hasWebSpeechSTT() ? 'browser' : 'openai';
        } else {
            this.sttMode = resolveSTTMode(this.manifest);
        }

        this.lang = DEFAULT_STT_LANG_SERVER;
        try { this.tx.mode = this.sttMode; this.tx.lang = this.lang; } catch { }
        ...
        if (this._mp3On()) { await this.ask.prefetch(topLevel); }
    ...
    VoiceCore.prototype.start = async function () {
        ...
        if (this._resumeResolver) { try { this._resumeResolver(); } catch { } this._resumeResolver = null; }
        this.stats = { YES: 0, NO: 0, NA: 0, NEXT: 0, SKIP: 0 };
        ...
        this.dockUI.setSub('Starting…');
        const name = window.app?.config?.user?.users_firstname?.trim() || '';
        const tpl = this.manifest.startText || 'Hello, {name}! Welcome to the safety assistant.';
        const text = tpl.replace(/\{name\}/g, name);
        await this.ask.ask({ text, mp3: this.manifest.start });
    ...
    VoiceCore.prototype.stop = async function () {
        ...
        try { this.rec && this.rec.stopVisualOnly && this.rec.stopVisualOnly(); } catch { }
        try { this.rec && this.rec.release && await this.rec.release(); } catch { }
        try { this.tx && this.tx.abortBrowser && this.tx.abortBrowser(); } catch { }
        if (stopBtn) {
            this._setStopBtnUI && this._setStopBtnUI('stop');
    ...
    VoiceCore.prototype.chooseCategoryLoop = async function () {
        ...
        while (this.running && this.catIdx < this.categories.length) {
            ...
            if (needConfirm) {
                ...
                if (normalized === 'YES') {
                    ...
                    if (this._mp3On() && AUDIO_PREFETCH.mode === 'category' && qmp3.length) {
        ...
        if (this._killed || this._stoppedByUser) return;
        ...
        if (this.manifest.finished || this.manifest.finishedText) {
            await this.ask.ask({
                text: (this.manifest.finishedText || 'Finished'),
                mp3: this.manifest.finished
            });
        }
    ...
    VoiceCore.prototype.categoryQuestionsLoop = async function (cat) {
        while (this.running && this.qIdx < cat.questions.length) {
            this._inQuestionCycle = true;
            try {
                await this._pauseGate();
                var q = this.currentQuestion();

                var nextQ = cat.questions[this.qIdx + 1];
                if (this._mp3On() && AUDIO_PREFETCH.lookahead > 0 && nextQ && nextQ.mp3) {
                ...
                await this._pauseGate();
                var needReplay = !!this._reaskOnResume;
                this._reaskOnResume = false;

                this.dockUI.setSub(needReplay ? 'Replaying question…' : 'Playing question…');

                this._askingQuestion = true;
                ...
                if (this.dom.result) {
                    this.dom.result.textContent = 'Answer: ' + normalized;
                    this.dom.result.className = 'ans-' + normalized;
                }
                this.addLog(q.text, normalized, cat);
                this.cb.onAnswer && this.cb.onAnswer({
                    categoryId: cat.id,
                    questionId: q.id,
                    questionText: q.text,
    ...
    VoiceCore.prototype.fillFieldsLoop = async function () {
        for (var i = 0; this.running && i < this.fields.length; i++) {
            ...
            this.dockUI.setSub('Listening…');
            await this._pauseGate();
            var out = await this.listenFreeformWithRetries(3, {
                text: (f.repeatText || this.manifest.repeatFieldText || this.manifest.repeatText || 'Please repeat the field value.'),
                mp3: (f.repeatMp3 || this.manifest.repeatField || this.manifest.repeat)
            });
    ...
    VoiceCore.prototype.listenAndClassifyWithRetries = async function (maxAttempts) {
        ...
        if (!this.running || this._killed) return { normalized: null, raw: '', confidence: 0 };
        for (var attempt = 0; attempt < maxAttempts; attempt++) {
            if (!this.running || this._killed) return { normalized: null, raw: '', confidence: 0 };
            var combined = '';
            var tr = { text: '', confidence: 0 };
            if (this.sttMode === 'browser') {
                this.dockUI.micHot(true);
                let viz = null;
                try { viz = await this.rec.startVisualOnly(); } catch { }
                try {
                    tr = await this.tx.recognizeBrowserOnce({ lang: DEFAULT_STT_LANG_BROWSER, timeoutMs: 10000 });
                } finally {
                    try { this.rec.stopVisualOnly(); } catch { }
                    this.dockUI.micHot(false);
                }
                combined = tr.text || '';
                const normProbe = classifyTranscript(combined);
                const lowConf = (tr.confidence || 0) < 0.5;
                const hardErr = ['audio-capture', 'not-allowed', 'service-not-allowed', 'no-speech'].includes(String(tr.error || ''));
                const useless = !combined || !normProbe || lowConf;

                if (hardErr) { browserFailures = 2; }
                else if (useless) { browserFailures++; }
            } else {
                this.dockUI.micHot(true);
                var rec = await this.rec.recordOnce();
                this.dockUI.micHot(false);
                if (rec.silence) {
                    // play repeat and retry
                    if (this.manifest.repeat || this.manifest.repeatText) {
                        this.dockUI.setSub('Playing repeat…');
                        await this.ask.ask({ text: (this.manifest.repeatText || 'Please repeat'), mp3: this.manifest.repeat });
                        this.dockUI.setSub('Listening…');
                    }
                    continue;
                }
                this.dockUI.setSub('Transcribing…');
                tr = await this.tx.transcribe(rec.blob, { lang: this.lang });
                combined = tr.text || tr.raw || '';
            }

            if (!this.running || this._killed) return { normalized: null, raw: '', confidence: 0 };

            if (this.sttMode === 'browser' && browserFailures >= 2 && this.allowOpenAIFallback) {
                this.sttMode = 'openai'; try { this.tx.mode = 'openai'; } catch { }
                attempt--;
                if (this.manifest.repeat || this.manifest.repeatText) {
                    this.dockUI.setSub('Playing repeat…');
                    await this.ask.ask({
                        text: (this.manifest.repeatText || 'Please repeat'),
                        mp3: this.manifest.repeat
                    });
                    this.dockUI.setSub('Listening…');
                }
                continue;
            ...
            if ((tr.confidence || 0) < 0.75 && (this.manifest.repeat || this.manifest.repeatText)) {
                this.dockUI.setSub('Playing repeat…');
                await this.ask.ask({
                    text: (this.manifest.repeatText || 'Please repeat'),
                    mp3: this.manifest.repeat
    ...
    VoiceCore.prototype.listenFreeformWithRetries = async function (maxAttempts, repeatNode) {
        maxAttempts = maxAttempts || 2;
        ...
        var browserFailures = 0;
        if (!this.running || this._killed) return { cmd: null, text: '', raw: '', confidence: 0 };
        for (var attempt = 0; attempt < maxAttempts; attempt++) {
            if (!this.running || this._killed) return { cmd: null, text: '', raw: '', confidence: 0 };
            var combined = '';
            var tr = { text: '', confidence: 0 };
            if (this.sttMode === 'browser') {
                ...
                combined = tr.text || '';
                if (!combined) browserFailures++;
            } else {
                this.dockUI.micHot(true);
                var rec = await this.rec.recordOnce();
                this.dockUI.micHot(false);
                if (rec.silence) {
                    var node = (repeatNode && typeof repeatNode === 'object')
                        ? repeatNode
                        : {
                            text: (this.manifest.repeatFieldText || this.manifest.repeatText || 'Please repeat'),
                            mp3: (repeatNode || this.manifest.repeatField || this.manifest.repeat)
                        };
                    if (node.text || node.mp3) {
                        this.dockUI.setSub('Playing repeat…');
                        await this.ask.ask(node);
                        this.dockUI.setSub('Listening…');
                    }
                    continue;
                }
            ...
            if (this.sttMode === 'browser' && browserFailures >= 2 && this.allowOpenAIFallback) {
                this.sttMode = 'openai'; try { this.tx.mode = 'openai'; } catch { }
                attempt--;
                var node2 = (repeatNode && typeof repeatNode === 'object')
                    ? repeatNode
                    : {
                        text: (this.manifest.repeatFieldText || this.manifest.repeatText || 'Please repeat'),
                        mp3: (repeatNode || this.manifest.repeatField || this.manifest.repeat)
                    };
                if (node2.text || node2.mp3) {
                    this.dockUI.setSub('Playing repeat…');
                    await this.ask.ask(node2);
                    this.dockUI.setSub('Listening…');
                }
                continue;
            }
            ...
            if ((tr.confidence || 0) < 0.75 && (node2.text || node2.mp3)) {
                this.dockUI.setSub('Playing repeat…');
                await this.ask.ask(node2);
                this.dockUI.setSub('Listening…');
            }
    ...
    VoiceCore.prototype._pauseGate = async function () {
        if (!this.paused) return;
        await new Promise((res) => { this._resumeResolver = res; });
    };

    VoiceCore.prototype.pause = async function () {
        if (!this.running || this.paused) return;
        this.paused = true;

        if (this.phase === 'in-category' && this._inQuestionCycle) {
            this._reaskOnResume = true;
        }
        ...
        try { this.rec && this.rec.stopVisualOnly && this.rec.stopVisualOnly(); } catch { }
        try { this.tx && this.tx.abortBrowser && this.tx.abortBrowser(); } catch { }
        try { Helper.hideLoader(); } catch { }

        var stopBtn = Helper.$('#stopBtn', this.host);
        if (stopBtn) {
            this._setStopBtnUI('stop');
            this.dockUI.setSub('Paused');
        }
    ...
    VoiceCore.prototype.resume = async function () {
        if (!this.running || !this.paused) return;
        this.paused = false;

        var stopBtn = Helper.$('#stopBtn', this.host);
        if (stopBtn) {
            this._setStopBtnUI('pause');
            this.dockUI.setSub('Resuming…');
        }
        ...
        var r = this._resumeResolver; this._resumeResolver = null;
        if (typeof r === 'function') r();
    };

    VoiceCore.prototype._setStopBtnUI = function (mode) {
        // mode: 'pause' | 'stop'
        var stopBtn = Helper.$('#stopBtn', this.host);
        if (!stopBtn) return;
        var svg = stopBtn.querySelector('svg.btn-icon');
        var lbl = stopBtn.querySelector('.btn-label');

        if (mode === 'pause') {
            if (svg) svg.innerHTML = '<rect x="6" y="5" width="4" height="14"></rect><rect x="14" y="5" width="4" height="14"></rect>';
            if (lbl) lbl.textContent = 'Pause';
            stopBtn.setAttribute('data-mode', 'pause');
        } else {
            if (svg) svg.innerHTML = '<path d="M6 6h12v12H6z"></path>';
            if (lbl) lbl.textContent = 'Stop';
            stopBtn.setAttribute('data-mode', 'stop');
        }
    };
    ...
    var VoiceSPA = (function () {
        ...
        var _handlers = { start: null, stop: null };
        var _mo = null;
        ...
        function _readConfig(host) {
            ...
            var endpoint = host.getAttribute('data-stt-endpoint') || 'openai_transcribe';
        ...
        async function mount(rootOrSelector, opts) {
            ...
            var ask = new AskMp3({ useTTSFallback: hasWebSpeech(), streaming: AUDIO_PREFETCH.streaming });
            ...
            _handlers.start = onStart;
            _handlers.stop = onStop;
            if (startBtn) startBtn.addEventListener('click', _handlers.start);
            if (stopBtn) stopBtn.addEventListener('click', _handlers.stop);
            ...
            async function onStart() {
                ...
                if (!instance.running) {
                    ...
                    if (stopBtn) {
                        ...
                        instance._setStopBtnUI && instance._setStopBtnUI('pause');
                    }

                    const startBtn = root.querySelector('#startBtn');
                    if (startBtn) startBtn.disabled = true;

                    instance.start().catch((e) => console.error('[voice] start failed:', e));
                    return;
                }
            ...
            async function onStop() {
                ...
                if (instance.running && instance.paused) {
                    ...
                    if (stopBtn) {
                        stopBtn.disabled = true;
                        instance._setStopBtnUI && instance._setStopBtnUI('stop');
                    }
                    return;
                }
        ...
        function unmount() {
            if (_mo) { try { _mo.disconnect(); } catch { } _mo = null; }

            if (mountedRoot) {
                try {
                    var sb = mountedRoot.querySelector('#startBtn');
                    var tb = mountedRoot.querySelector('#stopBtn');
                    if (sb && _handlers.start) sb.removeEventListener('click', _handlers.start);
                    if (tb && _handlers.stop) tb.removeEventListener('click', _handlers.stop);
                } catch { }
            }
            _handlers.start = _handlers.stop = null;
...
(function (global) {
    ...
    */
    global.voiceHelper = (function () {
        ...
        var mounted = false;
        var _mo = null;
        var _handlers = { start: null, stop: null, kick: null };
        ...
        function buildMarkup(manifestUrl, endpoint, uiMode) {
            ...
            return (
                ...
                '<svg class="btn-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><rect x="6" y="5" width="4" height="14"></rect><rect x="14" y="5" width="4" height="14"></rect></svg>' +
        ...
        return {
                        ...
                        if (mode === 'whenIncomplete') {
                            ...
                            if (shouldAutoStart && inst && !inst.running) {
                                ...
                                if (hasGesture) {
                                    ...
                                    _handlers.kick = onceKick;
                                    document.addEventListener('pointerdown', _handlers.kick, true);
            ...
            destroy: async function () {
                ...
                try {
                    if (rootEl) {
                        var sb = rootEl.querySelector('#startBtn');
                        var tb = rootEl.querySelector('#stopBtn');
                        if (sb && _handlers.start) sb.removeEventListener('click', _handlers.start);
                        if (tb && _handlers.stop) tb.removeEventListener('click', _handlers.stop);
                    }
                } catch (e) { console.warn('[voiceHelper] detach btn listeners error:', e); }

                try {
                    if (_handlers.kick) {
                        document.removeEventListener('pointerdown', _handlers.kick, true);
                        _handlers.kick = null;
                    }
                } catch (e) { console.warn('[voiceHelper] detach kick listener error:', e); }

                try { _mo && _mo.disconnect(); } catch (e) { console.warn('[voiceHelper] mo disconnect error:', e); }
                _mo = null;

                try {
                    var inst = (window.VoiceSPA && window.VoiceSPA.getInstance && window.VoiceSPA.getInstance()) || null;
                    if (inst) {
                ...
                } catch (e) { console.warn('[voiceHelper] instance destroy/stop error:', e); }

                try {
                    if (window.VoiceSPA && typeof window.VoiceSPA.unmount === 'function') {
                        window.VoiceSPA.unmount();
                    }
                } catch (e) { console.warn('[voiceHelper] VoiceSPA.unmount error:', e); }

                try {
                    var loader = rootEl && rootEl.querySelector('#loader');
                    if (loader) loader.classList.remove('show');
                    var micDot = rootEl && rootEl.querySelector('#micDot');
                    if (micDot) micDot.classList.remove('hot');
                } catch { }

                try {
                    document.querySelectorAll('.js-voice-start-cat').forEach(function (btn) {
                        btn.remove();
                    });
                    document.querySelectorAll('[data-voice-button-added]').forEach(function (el) {
                        try { delete el.dataset.voiceButtonAdded; } catch { }
                    });
                } catch (e) { console.warn('[voiceHelper] cleanup category buttons error:', e); }
                ...
                _handlers.start = _handlers.stop = _handlers.kick = null;
                try { $("#card-form").css({ "padding-bottom": "unset" }); } catch { }
            }
        };
    })();
