...
- Actual: Dedicated per-lookup settings/filter review is partially implemented. Generic lookup active filtering, preset lookup authoring controls, the shared dictionary option query contract, runtime named-dictionary filter enforcement, first-pass contact/company/project lookup View Filter authoring, and backend runtime-list compilation for single-value `contact_lookup`, `company_lookup`, and `project_lookup` user-scoped clauses now exist. Multivalue lookup filtering remains future work.
...
- Resolution: First Form Builder authoring pass added preset display-template selects and explicit preset filters for `Contact` / `Contacts`, `Company` / `Companies`, and `Project` / `Projects`. Preset filters now use shared tenant dictionary routes and UI Kit Combobox multiselects for `jobtypes`, `companies`, and `companyTypes` instead of raw ID text inputs. Named dictionaries use `GET /app/dictionaries/{dictionaryKey}/options` for unfiltered requests and `POST /app/dictionaries/options/query` when filters are present; generic ordinary lookup option sources also use `POST /app/dictionaries/options/query` with `sourceModel`, selected fields, filters, search, ids, and paging. Runtime form lookup requests now preserve `lookupConfig.filters[]` for named preset dictionaries, and backend dictionary sources enforce the supported filter columns for contacts, companies, and projects. Combobox filter pickers debounce remote search by 300 ms, load the first 10 options, and load additional pages on scroll. DB LOOKUP filter authoring and runtime static filter enforcement are complete for the current scope. Active-record filtering remains a runtime/default concern, not a preset Form Builder setting. View Filter authoring excludes multiple lookup fields and lookup-derived alias fields from the selectable filter target list. `Reported By`, `Contact`, and other single-value `contact_lookup` View Filters expose only `User Active Account` and `By User's Company` switches, both off by default; `Contact Job Type` is removed from the View Filter modal. Single-value `company_lookup` View Filters expose `Business Unit is User's Company` and `Main Company is User's Company` switches, both off by default; old Business Unit Type/Name/Division-style clauses are sanitized away. Single-value `project_lookup` View Filters expose `Project is in User's Access List`, off by default; old `assigned_projects`/Business Unit clauses are sanitized away. Runtime list compilation now ignores those user-scoped contact/company/project filters for root users, applies them for non-root users, OR-groups the same semantic clause across multiple single-value fields, ANDs different semantic groups, and compiles predicates through the data view so hidden/non-grid lookup fields can still filter rows. Company main-company filtering resolves through `company.main_company_id`; project access filtering resolves through `projectsaccess`.
- Fixed in: `c3bd9bf`, `8d68cb6`, the runtime lookup dictionary filter fix, the single-value `contact_lookup` runtime-list compiler fix, the single-value `company_lookup` runtime-list compiler fix, and the current single-value `project_lookup` runtime-list compiler fix.
- Verification: targeted backend dictionary/runtime tests, API client Vitest/typecheck, tenant-web typecheck/lint/Vitest, Browser Use runtime/Form Builder lookup smokes, `go test ./modules/tenant/platformstudioformbuilder`, `git diff --check`, docs/memory/env checks, and `scripts/preflight.sh` passed across the recorded slices.
