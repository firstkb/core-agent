...
Goal:
prepare the first real implementation plan for Platform Studio based on the new product direction.
...
Reference inputs:
- platform/frontend/docs/platform-studio/old-code-reference/** only if there is a real need to inspect previous technical behavior or extract a proven reusable helper
- platform/frontend/docs/platform-studio/EXTDB/** as a legacy product-behavior reference for details such as fields, filters, pages, hidden business rules, and operational model gaps; never as code to copy
- platform/frontend/docs/platform-studio/ezform/** as the main interaction reference for the future Form Builder; use it to study builder flow and editing ergonomics, never as a copy-paste implementation
- current tenant-web Platform Studio code only as technical reference

Hard rules:
- do not continue the current cockpit-style Platform Studio UI as the product baseline
- treat Forms and Navigation as the two primary builders
- Platform Studio should enter from the left rail area, not from the tenant sidebar
...
- avoid inventing extra top-level Platform Studio pages unless they are clearly necessary
...
```text
You are the orchestration owner for Platform Studio.
...
- platform/frontend/docs/platform-studio/old-code-reference/** may be consulted only when there is a real need to inspect previous technical behavior or reusable helpers
- platform/frontend/docs/platform-studio/EXTDB/** is a legacy product-behavior reference for details like fields, filters, pages, workflows, and hidden business requirements; do not copy code from it
- platform/frontend/docs/platform-studio/ezform/** is the main interaction reference for the future Form Builder; study interaction patterns and builder ergonomics, but do not copy-paste code or UI

Approved product direction:
- Platform Studio should focus first on two primary builders:
  - Forms
  - Navigation
- Platform Studio is entered from the left rail area, not from the tenant sidebar
...
- Avoid many top-level Platform Studio pages too early
- Do not continue the old cockpit-style builder UI
- Do not treat the retired Platform Studio implementation as the product baseline
...
Use this when the agent should archive the current Platform Studio implementation and remove it from the live product path.

```text
Work directly from repository docs and code. Do not use skill wrappers or workflow shortcuts.

Goal:
retire the current Platform Studio implementation from the live product direction, preserve only useful technical reference, and prepare the repo for Platform Studio.
...
- tenant shell integration and routing that exposes the current Platform Studio

Hard rules:
- preferred rule: do not keep live TypeScript or React source files in docs/
- owner-approved temporary exception: for this cleanup pass, it is acceptable to place a short-lived archive copy under `platform/frontend/docs/platform-studio/old-code-reference/`
...
- remove current Platform Studio from the tenant sidebar and other misleading live entry points if they imply a product direction we are abandoning
...
3. Write archive docs under:
   - platform/frontend/docs/platform-studio/old-code-reference/current-builder-file-map.md
   - platform/frontend/docs/platform-studio/old-code-reference/reusable-foundation-notes.md
   - platform/frontend/docs/platform-studio/old-code-reference/discarded-ui-patterns.md
...
5. If the owner still wants a temporary raw code archive, keep only the minimum raw snapshot material still needed under:
   - platform/frontend/docs/platform-studio/old-code-reference/code-snapshot/
