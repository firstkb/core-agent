...
// ApplyMaster runs migrations only against the master database.
func (m *Migrator) ApplyMaster(ctx context.Context) error {
	if m.client == nil || m.client.masterDB == nil {
		return fmt.Errorf("postgres: master DB is required for master migrations")
	}
	...
	if len(migrations) == 0 {
		m.logger.Info("migrator: no master migrations found", "dir", m.migrationsDir)
		return nil
	}

	if err := ensureSchemaMigrations(ctx, m.client.masterDB); err != nil {
		return fmt.Errorf("ensure master schema_migrations: %w", err)
	}

	applied, err := loadAppliedMigrations(ctx, m.client.masterDB)
	if err != nil {
		return fmt.Errorf("load applied master migrations: %w", err)
	}
	...
	for _, mig := range migrations {
		...
		if err := applyMigration(ctx, m.client.masterDB, mig); err != nil {
			return fmt.Errorf("apply master migration %s: %w", mig.Name, err)
		}
		m.logger.Info("migrator: applied master migration", "migration", mig.Name)
	}
...
func applyMigration(ctx context.Context, db *sql.DB, mig Migration) error {
	...
	if migrationOwnsTransaction(string(content)) {
		if _, err := db.ExecContext(ctx, string(content)); err != nil {
			return fmt.Errorf("exec migration: %w", err)
		}

		const insert = `INSERT INTO schema_migrations (version, applied_at) VALUES ($1, $2)`
		if _, err := db.ExecContext(ctx, insert, mig.Version, time.Now()); err != nil {
...
func migrationOwnsTransaction(content string) bool {
	normalized := strings.ToLower(content)
	return strings.Contains(normalized, "begin;") && strings.Contains(normalized, "commit;")
}
...
func discoverMigrationsDir() string {
	...
	cwd, err := os.Getwd()
	if err != nil {
		return "src/migrations/postgres/tenant"
	}

	candidates := []string{
		filepath.Join(cwd, "src", "migrations", "postgres", "tenant"),
		filepath.Join(cwd, "migrations", "postgres", "tenant"),
