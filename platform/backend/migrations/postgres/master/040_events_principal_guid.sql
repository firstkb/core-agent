...
BEGIN
  IF EXISTS (
    ...
    FROM pg_class
    WHERE relkind = 'i'
      AND relname = 'ix_events_actor_created'
  ) AND NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relkind = 'i'
      AND relname = 'ix_events_principal_created'
  ) THEN
    ALTER INDEX ix_events_actor_created RENAME TO ix_events_principal_created;
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS ix_events_principal_created
  ON events(events_principal_guid, events_created_at DESC)
