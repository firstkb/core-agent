...
func (svcHost *ServiceHost) RunService(service *HostedService, args []string) {
	defer func() {
		if r := recover(); r != nil {
			svcHost.Logger.Error(fmt.Sprintf("fatal error: %v", r), "stack", appinfo.StackWithoutPanic(debug.Stack()))
			os.Exit(1)
		}
	}()
...
func (sh *handler) Execute(args []string, cr <-chan svc.ChangeRequest, changes chan<- svc.Status) (ssec bool, errno uint32) {
	defer func() {
		if r := recover(); r != nil {
			sh.host.Logger.Error(fmt.Sprintf("fatal error in Service handler: %v", r), "stack", appinfo.StackWithoutPanic(debug.Stack()))
		}
	}()
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				sh.host.Logger.Error(fmt.Sprintf("fatal error: %v", r), "stack", appinfo.StackWithoutPanic(debug.Stack()))
			}
		}()
