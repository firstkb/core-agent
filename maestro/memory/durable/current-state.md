...
- `planned` `owner-confirmed`: Navigation Builder backend persistence, runtime publication, real rail utility visibility/access enforcement, and real ACL enforcement remain planned; Access in V1 is UI mock only unless a later decision splits Access Builder into its own tool.
