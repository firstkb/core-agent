...
// ApplyMasterMigrations runs SQL migrations for the master database only.
func (c *Client) ApplyMasterMigrations(ctx context.Context, logger *slog.Logger, opts ...MigratorOption) error {
	if c == nil || c.masterDB == nil {
		return fmt.Errorf("postgres: master DB is not initialized")
	}
	...
	return migrator.ApplyMaster(ctx)
}
