...
- Runtime create/edit/change handling must write audit entries into the tenant `events` table so owners can see what was created and changed. Autosave-style field changes must be coalesced by record and a short time/session window so one user edit burst becomes one meaningful event payload instead of many noisy rows. Exact event payload shape, merge window, and user-facing history UI remain a follow-up backend/runtime contract.
...
- Runtime `geo_point` should preserve the legacy smart-app behavior in a modern UI Kit control: if a `geo_point` field is present and empty, the form automatically attempts browser geolocation on open; if browser geolocation fails, it falls back to IP geolocation, with `ipapi.co` acceptable for the first slice behind a replaceable provider helper. The field is always readonly for manual typing regardless of authored schema/view visibility; runtime may still programmatically fill and save the value. The map button should be visually attached to the coordinate input as one input group and should open the selected coordinates in a map. Existing coordinate values must not be overwritten on form open.
- Runtime `view_only_field` with `viewOnlyBinding.kind = root_record_id` displays the source-table record id from the backend `recordId` field. It must not display the route `docGuid`.
