...
func (s *ServiceTenantProvider) validateSecure(ctx context.Context, routeID router.RouteID, r *http.Request) (*Tenant, error) {
	...
	if claim != nil && claim.TenantID == "" && hasScope(claim.Scope, auth.AccessScopeAdminAPI) {
		return nil, nil
	}
...
func (s *ServiceTenantProvider) decodeSurveyToken(code string) (formsvc.SurveyTokenPayload, error) {
	var zero formsvc.SurveyTokenPayload
	if s.tokenCodec == nil {
		return zero, ErrTokenCodecNotConfigured
	}

	payload, err := auth.DecodeToken(s.tokenCodec, formsvc.SurveyTokenSchema{}, code)
	if err != nil {
		if s.logger != nil {
