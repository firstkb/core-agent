...
- `maestro/memory/atlas/templates/` contains operational templates, not canonical project memory.
...
Core rules:
- Treat `maestro/memory/` as the compact project memory layer with different roles:
...
- Treat `maestro/memory/durable/canonical-docs.md` as the authority for which docs are canonical.
...
Memory update rules:
- Update `maestro/memory/durable/current-state.md` when progress materially changes the active project state.
- Update `maestro/memory/durable/decisions-log.md` when a durable decision is made or superseded.
- Update the relevant module pack under `maestro/memory/modules/**` when a contract or boundary changes.
- Update `maestro/memory/durable/canonical-docs.md` only when document authority changes.
