# UI Builder To Backend Boundary

## Purpose

This document defines the early boundary between the V2 UI Builder and the future backend implementation.
...
- UI may later say `Object` and `Screen`
...
The UI Builder should not write directly in database terms.

The UI Builder should author:
...
The UI Builder will need these backend-level capabilities.
...
- UI Builder may still create and edit views
...
The UI Builder should not own DDL rules directly.
...
The UI Builder should validate:
...
For the current V2 direction, preview can remain local-first in the UI Builder.

That means:

- the UI Builder may generate preview state locally
...
The first backend-ready UI Builder should be able to send and receive:
...
- UI Builder owns authoring intent
