...
type Server struct {
	...
	dictionaryHTTP                      *dictionary.Handler
...
func (srv *Server) Run(ctx context.Context) {
	srv.logger.Info(fmt.Sprintf("Tenant API server started, listening on %s", srv.httpServer.Addr))
...
func (srv *Server) initialize() error {
	...
	const (
		...
		defaultPoolMaxIdleTime = time.Duration(0)
	)
	...
	if srv.config.DB.PoolConfig.MaxIdleTime != "" {
		if d, err := time.ParseDuration(srv.config.DB.PoolConfig.MaxIdleTime); err == nil {
			...
			srv.logger.Warn("invalid DB pool max idle time, using default", "value", srv.config.DB.PoolConfig.MaxIdleTime, "error", err)
		}
	}
