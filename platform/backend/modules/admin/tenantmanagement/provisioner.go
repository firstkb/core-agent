...
func ensureSchemaMigrationsTable(db *postgres.Database) error {
	const ddl = `
...
func loadAppliedMigrations(db *postgres.Database) (map[string]bool, error) {
...
func (p *Provisioner) copyTable(ctx context.Context, source, target *postgres.Database, table string, tenantID int64) error {
	...
	exists, err = tableExists(target, table)
	if err != nil {
		return err
	}
	if !exists {
	...
	tenantColumn := tenantColumnForTable(table)
	query := fmt.Sprintf(`SELECT * FROM %s WHERE %s = $1`, pq.QuoteIdentifier(table), pq.QuoteIdentifier(tenantColumn))
	...
	pkColumns, err := loadPrimaryKeys(target, table)
	if err != nil {
		return err
	}
...
func tableExists(db *postgres.Database, table string) (bool, error) {
	const q = `
SELECT EXISTS (
...
func tenantColumnForTable(table string) string {
	switch table {
	case "users", "public_code", "notification_template":
		return "tenant_id"
	default:
		return table + "_tenant_id"
	}
}

func loadPrimaryKeys(db *postgres.Database, table string) ([]string, error) {
	const q = `
SELECT a.attname
  FROM pg_index i
