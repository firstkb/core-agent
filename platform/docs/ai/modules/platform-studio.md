...
- current Platform Studio preview entry route is `/app/platform-studio/forms/models/:modelId/views/:viewId`; this is a preview/authoring route namespace, not a second target type and not a Navigation Builder target
...
- current runtime entry split is now partially implemented: Form Builder `View data` opens the Platform Studio preview route `/app/platform-studio/forms/models/:modelId/views/:viewId`, while Navigation Builder and favorites continue to use `/app/forms/:modelId/views/:viewId`; backend now mirrors preview runtime APIs under `/app/platform-studio/forms/models/:modelId/views/:viewId/runtime/*`, and the remaining follow-up is to enforce the two dedicated guards on the runtime vs preview API namespaces
