import type { FormsPlaceholderObject } from "./forms-placeholder-data";
...
export function getFormsAuthoringAccess(
  ...
  object?: FormsPlaceholderObject | null,
): FormsAuthoringAccess {
  const isModelOwner = actor.role === "schemaOwner";
  const isReadonlyUser = actor.role === "readonly";
  const modelAllowsViewOnlyEditing = object?.canEditScreensOnly ?? true;
  const canManageStructure = isModelOwner;
  const canEditViews = isModelOwner || (actor.role === "viewEditor" && modelAllowsViewOnlyEditing);

  let summaryKey = "tenant.platformBuilder.forms.permissionSummary.manageAll";
  let summaryVariant: FormsPermissionSummaryVariant = "brand";

  if (isReadonlyUser) {
    summaryKey = "tenant.platformBuilder.forms.permissionSummary.readonly";
    summaryVariant = "neutral";
  } else if (!canEditViews) {
    summaryKey = "tenant.platformBuilder.forms.permissionSummary.viewAccessUnavailable";
    summaryVariant = "warning";
  } else if (!canManageStructure) {
    summaryKey = "tenant.platformBuilder.forms.canEditScreensOnly";
    summaryVariant = "info";
  }
  ...
  if (!canManageStructure) {
    structureRestrictionKey = isReadonlyUser
      ? "tenant.platformBuilder.forms.permission.readonly"
      : object?.isStructureLocked
        ? "tenant.platformBuilder.forms.permission.lockedModelOwnerOnly"
        : "tenant.platformBuilder.forms.permission.ownerOnlyStructure";
  }
  ...
  if (!canEditViews) {
    viewRestrictionKey = isReadonlyUser
      ? "tenant.platformBuilder.forms.permission.readonly"
      : "tenant.platformBuilder.forms.permission.viewAccessDisabled";
  }
