...
- `landed` `code-confirmed`: Navigation Builder has an active UI-first V1 surface at `/builder/navigation` with an app menu tree editor, root-only `Menu title` dividers, separate `App menu`/`Utility rail` left tabs, Form Builder-aligned action toolbar with save status, tab-first inspector with `Element`/`Access`, no V1 `Advanced` tab, `Element` divided into `Item`/`Opens`/`Channel`/`Warnings` sections with a `Show in app menu` toggle, inactive items shown by eye-off badges and marked hidden from runtime app menu output, Dashboard locked but without a lock badge, fixed add choices for `Menu title`, `Menu group`, `Form view`, `App page`, and `App module`, Form View labels derived continuously from selected View titles, Form View/App Page/External Link targets, future App Module shape with nested subitems, per-parent-level drag ordering, container/root add controls, container-level navigation icons only, mock-only access sheet, editable draft state, and explicit `Save`.
- `landed` `code-confirmed`: Navigation Builder backend persistence first slice is active in dedicated package `platformstudionavigationbuilder`, with `GET/PUT /app/platform-studio/navigation`, tenant table `ps_navigation_config`, optimistic version checks, duplicate target validation, and tenant bundle migration `007_platform_studio_navigation_builder.sql`.
- `planned` `owner-confirmed`: Navigation Builder runtime sidebar output, real rail utility visibility/access enforcement, and real ACL enforcement remain planned; Access in V1 is UI mock only unless a later decision splits Access Builder into its own tool.
