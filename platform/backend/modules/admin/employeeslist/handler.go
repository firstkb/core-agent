...
func (h *Handler) GetEmployee(ctx context.Context, r *http.Request, _ struct{}) (*EmployeeDetailResponse, error) {
	out, err := h.service.GetEmployee(ctx, strings.TrimSpace(r.PathValue("employeeId")))
	if err != nil {
		return nil, mapError(err)
	}
	return out, nil
...
func (h *Handler) UpdateEmployee(ctx context.Context, r *http.Request, req UpdateEmployeeInput) (*EmployeeDetailResponse, error) {
	out, err := h.service.UpdateEmployee(ctx, strings.TrimSpace(r.PathValue("employeeId")), req)
	if err != nil {
		return nil, mapError(err)
	}
	return out, nil
...
func mapError(err error) *apperr.AppError {
	...
	case errors.Is(err, ErrUnauthorized):
		return apperr.New("EMPLOYEES_LIST_UNAUTHORIZED", http.StatusUnauthorized, "unauthorized")
	case errors.Is(err, ErrForbidden):
		return apperr.New("EMPLOYEES_LIST_FORBIDDEN", http.StatusForbidden, "forbidden")
	case errors.Is(err, ErrInvalidQuery):
		return apperr.New("EMPLOYEES_LIST_INVALID_QUERY", http.StatusBadRequest, "invalid query")
	case errors.Is(err, ErrInvalidAction):
		return apperr.New("EMPLOYEES_LIST_INVALID_ACTION", http.StatusBadRequest, "invalid action")
	case errors.Is(err, ErrInvalidInput):
		return apperr.New("EMPLOYEES_LIST_INVALID_INPUT", http.StatusBadRequest, err.Error())
	case errors.Is(err, ErrEmployeeNotFound):
		return apperr.New("EMPLOYEES_LIST_EMPLOYEE_NOT_FOUND", http.StatusNotFound, "employee not found")
	case errors.Is(err, ErrEmployeeConflict):
		return apperr.New("EMPLOYEES_LIST_EMPLOYEE_CONFLICT", http.StatusConflict, "employee update conflict")
	case errors.Is(err, ErrSelfDeactivate):
		return apperr.New("EMPLOYEES_LIST_SELF_DEACTIVATE", http.StatusBadRequest, err.Error())
